...
import { galleryAlbumSchema } from "../../models";
...
export const getGalleryAlbums = once(async (): Promise<GalleryAlbum[]> => {
  ...
  const albums = await Promise.all(
    slugs.map(async (slug): Promise<GalleryAlbum> => {
      ...
      const assets = assetScope.album(slug);

      return {
        ...meta,
        cover: withAlt(resolveImage(assets, meta.cover), meta.name),
        images: withAltAll(resolveImages(assets, meta.images), meta.name),
        slug,
